...
DSs remain the source of truth. Durable additions, corrections, and human-verified summaries happen in DSs, not in DL.

### **Keep DL computed**

DL stores computed outputs, indexes, aggregations, categorization metadata, trust signals, freshness indicators, confirmation signals, and the catalog that locates them.

It should not become a manually maintained knowledge base. This holds even when a DL artifact physically lives in a DS (for example, an AI-generated summary in a Google Doc): such artifacts are computed output that happens to be stored in a DS, are provenance-marked as AI-generated, and are not the human-authored source of truth.
...
Identity is established by **Google SSO**. An email address is an identifier, not an authenticator — it must never be accepted as a self-asserted parameter at a trust boundary. For read-only access, MCP services require a **verified Google OIDC/OAuth token** (audience-validated); the verified email *claim* from that token is what determines which DS and DL data the user can access. The authenticated identity must be carried across each agent → MCP → DL hop (e.g., token passthrough or an on-behalf-of flow) so DL never trusts an unverified email string. This requirement applies equally to invocations from AI agents and from automation tools such as Zapier.
...
DSs and DL should be accessible through MCP services. MCP services are the main method AI tools use to query institutional data, and the main method skills use to write DL outputs to their chosen store.

This allows AI agents and AI-enabled applications to query DSs and DL in a consistent, permission-aware way. The goal is to make it easy for AI tools to discover useful knowledge quickly, with enough context to understand which DS records are relevant, trusted, current, and authorized.
...
- DSs → Deterministic Data Pipeline → DL (warehouse realization) → BI Dashboards
- DSs → AI Skill → DL (catalog + chosen backing store, via MCP)
- MCP Services ↔ DSs
- MCP Services ↔ DL
...
flowchart LR
  ...
  DS --> Pipeline[Data Pipeline] --> Warehouse["DL warehouse<br/>(deterministic path)"]
  Warehouse --> BI[BI Dashboards]

  DS_MCP --> Skill["AI Skill<br/>(Scheduled or Manual)"] --> DL_MCP

  subgraph DiscoveryLayer ["Discovery Layer (DL)"]
    DL_MCP[DL's MCP] === Catalog["Catalog<br/>(in a DS or store)"]
...
### **4.2 DL Population and Refresh**

* Access-control metadata is propagated from DSs into DL.
  * DL uses this metadata to enforce permissions when queried.
* Deterministic DL artifacts are updated by regular data pipelines.
...
* AI-assisted DL content is updated by scheduled or manually-run AI skills.
  ...
  * If source data has changed, moved, been deleted, or had access controls changed, the related DL content is updated, marked stale, or removed.
...
* AI agents, search tools, and AI-enabled applications query DSs and DL through MCP services.
  ...
  * Query results are filtered using DS permissions and DL access-control metadata.
  * Agents read the **catalog** to discover where each kind of derived output lives, then follow pointers to the relevant store.
  * **Enforcement tier:** where DL is materialized in a queryable store, the DL MCP service resolves the caller's group memberships and applies them as authorized query predicates — or uses BigQuery authorized views / row-access policies keyed to the SSO identity — so that restricted rows are never returned to the AI tool before access-control filtering. Where a DL artifact lives in a DS (e.g., a summary in a Doc), that DS's native permissions are the enforcement mechanism (see Section 7). Propagated group metadata is indexed to support this filtering.
* DL helps tools identify the best DS records to retrieve.
...
flowchart LR
  ...
  subgraph DiscoveryLayer ["Discovery Layer (DL)"]
    DL_MCP[DL's MCP] === DL["Catalog + backing stores"]
  end
...
DL is updated through two complementary mechanisms.

All DL updates should propagate or assign access-control metadata into DL, and register the output's location in the catalog.

### **Deterministic Data Pipelines**

For deterministic artifacts, such as dashboards and structured reporting outputs, a regular data pipeline updates DL — typically a warehouse realization — without AI involvement.
...
For AI-assisted or interpretation-heavy computed content, DL is updated by a scheduled or manually-run AI skill.
...
* Determining whether DL content needs updating.
...
* Rebuilding DL content when needed.
* Updating trust and confirmation metadata.

When rebuilding or refreshing DL content, the skill must check the staleness of referenced source data. If the DS content referenced by DL has changed, been deleted, moved, had access controls changed, or otherwise become stale, the relevant DL content should be updated, marked stale, or removed.

**Skill identity and scope.** The skill is the most privileged *reader* in the system: it reads broadly across DSs. Its read identity and scope must be defined explicitly and per DS, following least privilege — only the read access required for the DSs it processes. The skill must capture each item's **source access-control metadata at read time** so that every derived DL artifact is stamped with the correct ACL; failure to do so silently widens access.
...
The primary query path is through MCP services connected to DSs and DL. Tools read the DL catalog to locate the derived data they need, then follow pointers to the relevant store.

Examples include:

### **Simple AI Agent**

A locally run Claude Cowork-style agent with connectors or MCP services for approved DSs and DL. It can answer user questions by querying DSs and using DL to quickly identify likely relevant records.
...
These products may provide enterprise search, AI answers, knowledge discovery, and content generation over connected DSs and DL.
...
These platforms may provide an internal search and retrieval layer over DSs, optionally using DL to improve relevance, trust, freshness, and performance.

### **Third-Party Integration Trust Boundary**

Commercial and self-hosted tools are a distinct trust zone, not implicit insiders. Because DL aggregates institutional knowledge across many DSs, connecting an external tool to it without controls creates a path for bulk re-export of data that was individually restricted at the source. For each external consumer, define:

* **Credential scope** — how the tool authenticates and the least-privilege slice of DSs/DL it may read.
* **Data minimization** — which portion of DL the tool may access, rather than the whole of it.
...
Note that external tools typically query under their own service credentials and proxy the end-user's identity; the integration must ensure the tool faithfully represents the requesting user so DL enforcement is not bypassed.
...
* Access-control metadata is propagated from DSs into DL.
* DL enforces access based on the propagated metadata where it controls the store; where a DL artifact lives in a DS, that DS's native permissions enforce access.
...
This is **harder, not easier, when the artifact lives in a plain DS** (a Google Doc, a Sheet): that store offers only its own sharing settings, with no query-time most-restrictive-intersection enforcement like an authorized view provides. A sensitive aggregation written to a Doc is protected only by that Doc's sharing, which a skill or human must set correctly — an error-prone path. Therefore: aggregations whose mosaic sensitivity exceeds what the target store can enforce should **not** be materialized there. Instead, DL stores **instructions/pointers** directing permitted users to query the underlying DS under their own SSO identity to reconstruct or compute the fuller aggregation at query time. This keeps the sensitive computation under the user's real permissions and keeps DL recomputable.
...
DL may later index, categorize, aggregate, or point to those summaries, but the summary itself should live in a DS.

### **AI-Generated Artifacts in DSs**

A skill may write a **computed, human-readable artifact** — an AI-generated summary, digest, or curated index — into a DS where people read it (a Google Doc, a Confluence page). This is DL output that happens to be stored in a DS, and it must be:
...
DL may capture **data confirmations**.
...
Without DL, each AI agent or AI-enabled application may need to independently search across many DSs. This increases:
...
5. Propagate access-control metadata from DSs into DL.
6. Realize DL in the lightest store that fits: a **catalog-in-a-DS** (Google Sheet or Confluence page) plus a small number of computed-output locations, deferring a warehouse until scale demands it.
...
8. Build a scheduled or manually-run AI skill that updates AI-assisted DL content and registers each output in the catalog.
9. Include source-data and catalog-pointer staleness checks in the DL refresh process.
...
The MVP should prove that DL improves retrieval quality, speed, and trust without creating a second source of truth. Because the catalog-in-a-DS realization shrinks the custom build to "a catalog plus skills that write via MCP," it also lowers the stakes of the build-vs-buy decision noted in the open questions below.
