...
**Key terms.** The core concepts — **Data Sources (DSs)**, **Discovery Layer (DL)**, **Catalog**, the **DL-creation skill**, the **Query skill**, and **confirmation signals** — are defined in plain language in [lik-overview.md](lik-overview.md). One term used heavily below and not in that overview:
...
**Progressive disclosure.** Together the Catalog and DL let an agent find an answer in cheap steps instead of loading everything at once: consult the catalog to learn *what exists and where* (one lookup), follow the pointer to a DL output (a summary, index, or signal already distilled from the sources), and open the full DS records only when the question demands them. Each step costs more than the one before it, and most questions are answered before reaching the bottom.
1. **Catalog** (the entry point):  AI queries and get the high-level roadmap
1. **DL** (selective branching): AI identifies which individual concepts to look at
1. **DS** (traversing related data): Once inside a specific data record, AI discovers links to other data records
1. **On-Demand Graph Discovery**: AI then selectively follows those links to expand its understanding of the knowledge graph
