...
A **`human-created` DS record** — durable, not recomputable, attributed in version history — written under the **user's own SSO** into a DS-hosted store (a Confluence page or Google Doc). Unlike a DL-creation skill's output it carries **no** `discovery-layer` tag — that tag marks skill-produced derived material, and a saved answer is authored by a person — so it's a DS record, **not a DL record**. (DL's only contribution is the optional Catalog pointer below.) Because a synthesis can draw on records with different restrictions, it can **blend tiers**, so it isn't covered by §1.2's single-location inheritance. **The saving user sets the artifact's audience** when they save it, under their own SSO — and is **responsible for choosing one no broader than the sources it drew on** (default-deny if they specify nothing). The skill may surface the sources' restrictions to inform that choice, but access is the user's call, not the skill's. (Access model: <u>Architecture</u> §4; <u>Access Control</u>.)
